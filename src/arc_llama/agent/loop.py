...
from arc_llama.agent.prompts import SYSTEM_PROMPT, USER_INSTRUCTIONS, format_tool_result
...
async def run_agent(
    ...
    *,
    ...
    async with httpx.AsyncClient(timeout=timeout, base_url=base_url.rstrip("/")) as client:
        ctx = ToolContext(root=root, client=client, extra={"chat_store": chat_store})
